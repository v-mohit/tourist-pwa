...
const CafeCard = ({ item, index, buildImageUrl, router }: any) => {
  ...
  const entryFeeCard = ticketComponent?.card?.find(
    (c: any) =>
  ...
  const timingComp = placeDetails.find(
    (x: any) =>
      x.__typename === "ComponentPlaceDetailPlaceTime" ||
      x.__typename === "ComponentPlaceDetailPlaceothers",
  );
  ...
  const desc =
    ticketComponent?.information?.[0]?.description ||
    attributes?.description ||
    "Experience local flavors and stunning views.";
  ...
  return (
    <div className="cafe-card relative group flex flex-col">
      <div
      ...
      >
        <div className="cafe-img">
          <div className="dimg" style={{ backgroundImage: `url('${image}')` }} />
        ...
        <div className="cafe-body">
          <h4>{name}</h4>
          <p className={isExpanded ? "expanded" : ""}>{desc}</p>
          {!isExpanded && desc.length > 30 && (
            <button
              className="cafe-more-btn"
              onClick={(e) => {
                e.stopPropagation();
                setIsExpanded(true);
              }}
      ...
      <div className="cafe-foot">
        <div className="flex-1">
          {price && <div className="cafe-price">Starting from {price}</div>}
          {time && (
            <div
              style={{
                fontSize: 10,
  ...
  return (
    <section className="sec" id="cafeteria">
      <div className="sec-hd rv in">
        ...
        <Link
          href={`/tourist-attraction?categoryId=${
            data?.category?.data?.id || ""
          }&categoryName=${encodeURIComponent(data?.category?.data?.attributes?.Name || "Cafeteria")}`}
          className="see-all"
        >
          See all →
        </Link>
