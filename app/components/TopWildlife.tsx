...
const TopWildlife = ({ wildlife }: TopWildlifeProps) => {
  const places = wildlife?.category?.data?.attributes?.places?.data || []
  ...
  return (
    <section id="wildlife" className="px-6 md:px-8 py-16 md:py-24 max-w-6xl mx-auto">
      <div className="mb-12">
        ...
        <p className="text-base md:text-lg text-[#7A6A58] max-w-2xl leading-relaxed">
          Experience the wild beauty and exotic wildlife of Rajasthan's nature reserves
        </p>
      </div>
        ...
        {displayPlaces.map((place) => {
          ...
          return (
            <Link
              key={place.id}
              href={`#`}
              className="group overflow-hidden rounded-[16px] shadow-[0_4px_20px_rgba(24,18,14,0.1)] hover:shadow-[0_12px_48px_rgba(24,18,14,0.16)] transition-all duration-300"
            >
